...
void mapHack::GenerateStairsDown()
{
	bool didit = false;
...
void mapHack::GenerateObjects()
{
	for (int i = 0; i < 10; i++)
	{
		...
		while (!didit)
		{
			...
			if (WallAt(x, y) == WALL_Room && MapTile(x, y).entity == NULL)
			{
				objBase *object = objManager::GetInstance()->RandomItem(
						m_depth);
				if (object)
				{
...
void mapHack::MakeCorridors()
{
	...
	if (m_roomCount > 2)
	{
		...
		for (int i = (hnRandom::GetInstance()->Get(m_roomCount)) + 2; i > 0;
				i--)
		{
...
void mapHack::Join(uint8 roomA, uint8 roomB, bool extraCorridor)
{
	bool success = false;
...
bool mapHack::DigCorridor(const hnPoint2D &f, const hnPoint2D &t)
{
	// f is from, t is to.
	bool success = true;
	...
	while (corridor != t)
	{
		...
		if (corridor.x >= m_width || corridor.x < 0 || corridor.y >= m_height
				|| corridor.y < 0)
		{
			success = false;
			break;
...
void mapHack::FindEntranceInRect(hnPoint2D *result, const hnPoint2D & topLeft,
		const hnPoint2D & bottomRight)
{
	sint8 x, y;

	x = (topLeft.x == bottomRight.x) ?
			topLeft.x :
			(hnRandom::GetInstance()->Get(bottomRight.x - topLeft.x))
					+ topLeft.x;
	y = (topLeft.y == bottomRight.y) ?
			topLeft.y :
			(hnRandom::GetInstance()->Get(bottomRight.y - topLeft.y))
					+ topLeft.y;
...
bool mapHack::CreateRoom(sint8 x, sint8 y, sint8 w, sint8 h, sint8 xalign,
		sint8 yalign, bool lit)
{
...
bool mapHack::OkayForDoor(uint8 x, uint8 y)
{
	bool okay = false;
...
bool mapHack::NearDoor(uint8 x, uint8 y)
{
	bool nearDoor = false;
...
bool mapHack::CheckRoomOkay(const mapRoom &room)
{
	bool roomOkay = true;
...
bool mapHack::CheckTileOkayForRoom(uint8 x, uint8 y)
{
	bool tileOkay = true;
