...
objManager::objManager()
{
	...
	for (int i = 0; i < m_objectCount; i++)
	{
		if (m_objectDef[i].name)
			HN_Logger::LogInfo("%s  -  %d", m_objectDef[i].name,
					m_objectDef[i].probability);
		else
			HN_Logger::LogInfo("Unnamed - %d", m_objectDef[i].probability);
		m_objectTotalProbability[m_objectDef[i].type] +=
				m_objectDef[i].probability;
	}

	for (int i = 0; i < OBJ_TYPE_MAX; i++)
	{
		HN_Logger::LogInfo("Item class %d - %d total.", i,
				m_objectTotalProbability[i]);
	}
}
...
{ 4, OBJ_TYPE_Wand },
{ 3, OBJ_TYPE_Ring },
{ 1, OBJ_TYPE_Amulet } }; // probabilities MUST total 100!
