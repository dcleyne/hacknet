...
entBase::entBase(entType type, const hnPoint & pos, hnPlayer *player) :
		m_type(type), m_position(pos), m_wieldedObject(NULL), m_player(player)
{
...
void entBase::GetFullName(char * buffer, int bufferSize)
{
	if (m_player)
		snprintf(buffer, bufferSize, "%s the %s", m_player->GetName(),
				GetName());
	else
...
void entBase::SetPosition(const hnPoint & pos)
{
	m_position = pos;
}

void entBase::DoEffect(uint8 property, uint8 bc, objType type)
{
	...
	case PROP_Gain_Ability:
		...
		else
		{
			...
			else
			{
				...
				for (int j = 0; j < 6; j++)
				{
					int i = hnRandom::GetInstance()->Get(
							hnStatus::MAX_STATISTICS);
	...
	case PROP_Restore_Ability:
		...
		{
			sprintf(buffer, "Wow, this makes you feel %s",
					blessed ? "great" : "good");
		}
		break;
	case PROP_Paralyse:
	...
	case PROP_Heal:
		...
		GetStatus()->Heal(hnRandom::GetInstance()->Dice(6 + 2 * blessFactor, 4),
				(!cursed) ? 1 : 0, !blessed, !cursed);
		GetStatus()->ExerciseStatistic(hnStatus::Constitution, true);
		break;
	case PROP_Extra_Heal:
		sprintf(buffer, "You feel much better.");
		GetStatus()->Heal(hnRandom::GetInstance()->Dice(6 + 2 * blessFactor, 8),
				blessed ? 5 : ((!cursed) ? 2 : 0), !blessed, !cursed);
	...
	case PROP_Gain_Level:
		if (cursed)
		{
			if (GetPosition().z == 0)
			{
				// go up a level.
				sprintf(buffer,
						"You rise up through the ceiling! (Not really)");
			}
			else
			{
...
void entBase::DoEat(sint16 nutrition)
{
...
bool entBase::IsValidMove(hnDirection dir)
{
	hnPoint point;
...
bool entBase::IsValidTake(objBase *object)
{
...
bool entBase::Take(objBase *object, uint8 count)
{
	bool result = false;
	...
	if (IsValidTake(object))
	{
		...
		if (map) // this should never fail.
		{
			hnPoint pos = GetPosition();
			objBase *taken =
					map->MapTile(pos.x, pos.y).object->RemoveObjectQuantity(
							object, count);
		...
		{
			HN_Logger::LogInfo(
					"Weird -- GetTakeTarget() worked, but Take() didn't.");
		}
	}
	return result;
}

bool entBase::IsValidInventoryItem(objBase *object)
{
	bool result = false;
...
bool entBase::Drop(objBase *object, uint8 count)
{
	bool result = false;
	if (IsValidInventoryItem(object))
	{
		if (!object->IsWorn())
		{
			...
			if (map) // this should never fail.
			{
				objBase *dropped = m_inventory->RemoveObjectQuantity(object,
						count);
...
bool entBase::Wield(objBase *object)
{
	bool result = false;
...
bool entBase::Wear(objBase *object)
{
	bool result = false;
	if (IsValidInventoryItem(object))
	{
...
bool entBase::Remove(objBase *object)
{
	bool result = false;
	if (IsValidInventoryItem(object))
	{
...
bool entBase::Quaff(objBase *object)
{
	bool result = false;
	if (IsValidInventoryItem(object))
	{
...
bool entBase::Eat(objBase *object)
{
	bool result = false;
	if (IsValidInventoryItem(object))
	{
...
bool entBase::IsValidMoveDestination(const hnPoint &destination)
{
	...
	if (hnDungeon::GetLevel(destination.z)->WallAt(destination.x,
			destination.y) & WALL_Passable)
		if (hnDungeon::GetLevel(destination.z)->MapTile(destination.x,
				destination.y).entity == NULL)
			legalMove = true;

	return legalMove;
}

bool entBase::FindMoveDestination(hnPoint &destination, hnDirection dir)
{
	bool legalMove = false;
	...
	else // dir == DIR_Up || dir == DIR_Down -- we can't get here otherwise.  (hnGame::ClientMove() ignores other 'directions')
	{
		...
		if (dir == DIR_Up)
		{
			if (hnDungeon::GetLevel(currentPos.z)->WallAt(currentPos.x,
					currentPos.y) & WALL_StairsUp)
			{
				if (currentPos.z - 1 >= 0) // if we're not leaving the dungeon...
				{
					hnPoint2D stairPos =
							hnDungeon::GetLevel(currentPos.z - 1)->GetDownStairs();
		...
		{
			if (hnDungeon::GetLevel(currentPos.z)->WallAt(currentPos.x,
					currentPos.y) & WALL_StairsDown)
			{
				hnPoint2D stairPos =
						hnDungeon::GetLevel(currentPos.z + 1)->GetUpStairs();
				potentialPos.Set(stairPos.x, stairPos.y, currentPos.z + 1);
			}
		}
	}
...
bool entBase::RollToHit(entBase *target)
{
...
sint16 entBase::RollDamage(entBase *foe)
{
	int damage = 1;
...
void entBase::Move(hnDirection dir)
{
...
void entBase::MoveTo(const hnPoint & pos)
{
	//------------------------------------------------
...
bool entBase::IsValidAttack(hnDirection dir)
{
	bool valid = false;
...
int entBase::Attack(hnDirection dir)
{
	int result = AT_Miss;
