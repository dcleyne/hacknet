...
hnGroup::hnGroup(int maxPlayers) :
		m_player(NULL), m_monsterCount(0), m_playerCount(0), m_maxPlayerCount(
				maxPlayers)
{
	//---------------------------------------------------------
...
#define MONSTER_ATTRACT_DISTANCE_SQ (MONSTER_ATTRACT_DISTANCE*MONSTER_ATTRACT_DISTANCE)
				if (DistanceFromGroup(
						hnPoint(x, y, level)) <= MONSTER_ATTRACT_DISTANCE_SQ)
				{
...
int hnGroup::DistanceFromGroup(hnPlayer * player)
{
	//--------------------------------------------------------
...
int hnGroup::DistanceFromGroup(const hnPoint & where)
{
	//--------------------------------------------------------
	//  Find the minimum distance from any of the members of
...
void hnGroup::AddPlayer(hnPlayer * player)
{
	//------------------------------------------------------
...
void hnGroup::RemovePlayer(hnPlayer * player)
{
	assert(player);
	//------------------------------------------------------
...
void hnGroup::RemoveEntity(entBase * entity)
{
	//------------------------------------------------------
...
void hnGroupManager::Startup(int maxPlayers)
{
...
hnGroupManager::hnGroupManager(int maxPlayers) :
		m_group(NULL), m_maxGroupCount(maxPlayers)
{
...
void hnGroupManager::AddPlayer(hnPlayer * player)
{
...
void hnGroupManager::PutPlayerInGroup(int id)
{
...
void hnGroupManager::RemovePlayer(hnPlayer *player)
{
	for (int i = 0; i < m_maxGroupCount; i++)
	{
		m_group[i]->RemovePlayer(player);
...
void hnGroupManager::RemoveEntity(entBase *entity)
{
	for (int i = 0; i < m_maxGroupCount; i++)
	{
...
bool hnGroupManager::ProcessTurn()
{
...
void hnGroupManager::UpdateGroups()
{
