...
hnPlayer::hnPlayer(int playerID, const hnPoint &where) :
		m_playerID(playerID), m_lastSentGroupPlayerCount(0), m_lastSentGroupPlayerQueuedTurns(
				0), m_movePending(false), m_statsChanged(true), m_hitPointsChanged(
				true), m_spellPointsChanged(true)
{
...
//	m_entity = new entGridBug( where, this );	
	...
	hnDungeon::GetInstance()->GetLevel(where.z)->PutEntityAt(m_entity, where.x,
			where.y);
...
void hnPlayer::SetEntity(entBase * entity)
{
...
bool hnPlayer::IsValidMove(hnDirection dir)
{
	if (!m_entity)
...
bool hnPlayer::IsValidAttack(hnDirection dir)
{
	return m_entity->IsValidAttack(dir);
}

bool hnPlayer::IsValidInventoryItem(const objDescription &desc,
		uint8 inventorySlot)
{
	bool result = false;
...
bool hnPlayer::IsValidTake(const objDescription &desc, uint8 stackID)
{
...
hnPlayer::GetTakeTarget(const objDescription &desc, uint8 stackID)
{
	...
	if (map)
	{
		...
		objBase *realObject = map->MapTile(pos.x, pos.y).object->GetObject(
				stackID);
...
void hnPlayer::Move(hnDirection dir)
{
	if (IsValidMove(dir))
	{
...
void hnPlayer::Attack(hnDirection dir)
{
...
void hnPlayer::Take(const objDescription &desc, uint8 stackID)
{
...
void hnPlayer::Drop(const objDescription &object, uint8 inventorySlot)
{
	...
	{
		HN_Logger::LogInfo("Player requested illegal drop of inventorySlot %d",
				inventorySlot);
	}
}

void hnPlayer::Wield(const objDescription &object, uint8 inventorySlot)
{
	if (IsValidInventoryItem(object, inventorySlot))
	{
...
void hnPlayer::Wear(const objDescription &object, uint8 inventorySlot)
{
	if (IsValidInventoryItem(object, inventorySlot))
	{
...
void hnPlayer::Remove(const objDescription &object, uint8 inventorySlot)
{
	if (IsValidInventoryItem(object, inventorySlot))
	{
...
void hnPlayer::Quaff(const objDescription &object, uint8 inventorySlot)
{
	if (IsValidInventoryItem(object, inventorySlot))
	{
...
void hnPlayer::Eat(const objDescription &object, uint8 inventorySlot)
{
	if (IsValidInventoryItem(object, inventorySlot))
	{
...
void hnPlayer::Listen(const hnPoint & position, char * message)
{
	//---------------------------------------------------------
...
void hnPlayer::Listen(char * message)
{
	//---------------------------------------------------------
...
void hnPlayer::See(const hnPoint & where, entBase * who, char * message)
{
	if (CanSee(where))
	{
...
void hnPlayer::See(const hnPoint & where, char * message)
{
	if (CanSee(where))
	{
...
bool hnPlayer::CanSee(const hnPoint & where)
{
...
bool hnPlayer::HasQueuedTurn()
{
	return (m_queuedTurn.type != queuedTurn::None);
}

void hnPlayer::SetName(char * name)
{
...
void hnPlayer::GetFullName(char * buffer, int bufferSize)
{
	if (m_entity)
...
void hnPlayer::DoAction()
{
	...
	case queuedTurn::Move:
		m_movePending = m_entity->FindMoveDestination(m_moveDestination,
				m_queuedTurn.move.direction);
		success = true;
		break;
	...
	case queuedTurn::Take:
		success = m_entity->Take(m_queuedTurn.take.object,
				m_queuedTurn.take.takeCount);
		break;
	case queuedTurn::Drop:
		success = m_entity->Drop(m_queuedTurn.drop.object,
				m_queuedTurn.drop.dropCount);
		break;
	...
	case queuedTurn::Remove:
		...
		if (m_queuedTurn.remove.object)
			m_queuedTurn.remove.object->FillDescription(
					m_queuedTurn.remove.desc);
		break;
...
void hnPlayer::RecalculateVision()
{
	//---------------------------------------------------------------
	...
	if (m_map[m_entity->GetPosition().z] == NULL)
		m_map[m_entity->GetPosition().z] = new mapClient(realMap->GetWidth(),
				realMap->GetHeight());
...
void hnPlayer::SendUpdate()
{
	...
	if (m_group)
	{
		...
		{
			netServer::GetInstance()->SendGroupData(groupMembers,
					groupMembersWithTurns, HasQueuedTurn());
				...
				for (int j = 0; j < INVENTORY_MAX; j++)
				{
					if (m_clientInventoryMapping[j] == object)
					{
						...
						if (!object->ExactMatch(m_clientInventory[j]))
						{
							...
							netServer::GetInstance()->SendInventoryItem(
									m_clientInventory[j], j);
							HN_Logger::LogInfo(
									"Sending updated inventory slot %d", j);
						}
						break;
					}
				}
				...
				if (!foundMatch)
				{
					...
					for (int j = 0; j < INVENTORY_MAX; j++)
					{
						if (m_clientInventoryMapping[j] == NULL
								&& m_clientInventory[j].type == result.type)
						{
							updatedID = j;
							finished = true;
	...
	{
	case queuedTurn::Take:
		netServer::GetInstance()->SendTakenItem(m_completedTurn.take.desc,
				updatedID);
		break;
	case queuedTurn::Drop:
...
void hnPlayer::RefreshMap(int level)
{
	netServer::GetInstance()->StartMetaPacket(m_playerID);
...
void hnPlayer::SendMapData(const hnPoint2D &tlpos, const hnPoint2D &brpos,
		int level)
{
	...
	if (tlpos.x <= brpos.x && tlpos.y <= brpos.y)
	{
		...
		for (int j = 0; j < update.height; j++)
			for (int i = 0; i < update.width; i++)
			{
				...
				update.entityType[i + (j * update.width)] =
						map->MapTile(x, y).entity;
				...
				for (int k = 0; k < objectCount; k++)
					update.object[i + (j * update.width)][k] = map->MapTile(x,
							y).object[k];
			}
