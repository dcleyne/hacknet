...
mapBase::mapBase(uint8 width, uint8 height, uint8 depth) :
		m_width(width), m_height(height), m_depth(depth), m_roomCount(0)
{
	m_stairsUp.Set(-1, -1);
...
bool mapBase::OnMap(uint8 x, uint8 y)
{
	if (x < m_width && y < m_height)
		return true;
...
void mapBase::RemoveObject(objBase *object)
{
...
void mapBase::PutObjectAt(objBase *object, uint8 x, uint8 y)
{
	mapTile & tile = MapTile(x, y);
	tile.object->AddObject(object);
}

void mapBase::MoveObjectTo(objBase *object, uint8 x, uint8 y)
{
	RemoveObject(object);
	PutObjectAt(object, x, y);
}

void mapBase::RemoveEntity(entBase *entity)
{
...
void mapBase::PutEntityAt(entBase *entity, uint8 x, uint8 y)
{
	mapTile & tile = MapTile(x, y);
	tile.entity = entity;
}

void mapBase::MoveEntityTo(entBase *entity, uint8 x, uint8 y)
{
	RemoveEntity(entity);
	PutEntityAt(entity, x, y);
}

void mapBase::Generate()
{
...
void mapBase::CalculateVisibility(const hnPoint & position, mapClient * destMap)
{
	//-------------------------------------------------------------
	//
	...
	for (int i = 0; i < m_width; i++)
		for (int j = 0; j < m_height; j++)
		{
			...
			else if ((fabs(i - position.x) <= 1) && (fabs(j - position.y) <= 1))
				MapTile(i, j).lit = true;
		}
...
void mapBase::UpdateMap(mapClient * destMap)
{
	//-------------------------------------------------------------------
...
void mapBase::MarkPointChanged(uint8 x, uint8 y)
{
...
bool mapBound::inUse() const
{
	return (max != 0.0f);
}

bool mapBound::reached() const
{
	return (inUse() && count > max);
}

bool mapBound::steeper(float inverseGradient) const
{
	return (!inUse() || inverseGradient < max);
}

bool mapBound::shallower(float inverseGradient) const
{
...
void mapBase::CalculateOctantVisibility(const hnPoint &position,
		const hnPoint &offset, bool swap, unsigned int N, mapClient *destMap)
{
	bool blocker;
	int below = 0;
	...
	for (col = 1; col < N; ++col)
	{
		...
		for (cell = 0; cell <= col; ++cell)
		{
			...
			if (swap)
			{
				t = &MapTile(position.x + offset.x * cell,
						position.y + offset.y * col);
				dt = &destMap->MapTile(position.x + offset.x * cell,
						position.y + offset.y * col);
				onMap = OnMap(position.x + offset.x * cell,
						position.y + offset.y * col);
			}
			else
			{
				t = &MapTile(position.x + offset.x * col,
						position.y + offset.y * cell);
				dt = &destMap->MapTile(position.x + offset.x * col,
						position.y + offset.y * cell);
				onMap = OnMap(position.x + offset.x * col,
						position.y + offset.y * cell);
			}
			...
			if (blocker)
			{
				...
				if ((below >= 0 && m_cells[below].lit) || m_cells[cell].lit
						|| belowLeftLit)
				{
			...
			if (below >= 0)
			{
				...
				if (m_cells[below].lowerBound.reached())
				{
					...
					m_cells[cell].visibleBlocker = true;
					m_cells[cell].lowerBound.max =
							m_cells[below].lowerBound.max;
					m_cells[cell].lowerBound.count =
							m_cells[below].lowerBound.count
									- m_cells[below].lowerBound.max;
				...
				if (m_cells[below].upperBound.reached())
				{
					...
					m_cells[cell].lit = false;
					m_cells[cell].visibleBlocker = false;
					m_cells[cell].upperBound.max =
							m_cells[below].upperBound.max;
					m_cells[cell].upperBound.count =
							m_cells[below].upperBound.count
									- m_cells[below].upperBound.max;
			...
			if (onMap)
			{
				...
				if (m_cells[cell].lit
						|| (blocker && m_cells[cell].visibleBlocker))
				{
