...
public:
	...
	{
	} // called after submittting a turn.
	...
	virtual void UpdateMapTile(const hnPoint &point,
			const mapClientTile & tile);
	virtual void UpdateMapCreature(const hnPoint &point, entType type);
	virtual void UpdateGroupData(int groupMemberCount, int groupMemberTurnCount,
			bool submittedTurn);
	...
	void GetInventoryItemText(int inventoryItem, char *buffer,
			int bufferLength);
