...
public:
	...
	virtual bool Go();
	virtual bool Stop(); // terminate the main run loop and clean up
	...
	void DrawObjectArrayFiltered(objDescription *array, uint8 count,
			uint16 flagFilter, bool inventory);
	...
	virtual void UpdateGroupData(int groupMembers, int groupMembersWithTurns,
			bool haveISubmitted);
